...
function DemandeQuickActions({
  ...
  const isMine =
    currentUserId && String(demande.assigned_to) === String(currentUserId);
  const canTakeCharge = !isClosed && (!isMine || demande.status === "nouvelle");
    ...
    >
      {canTakeCharge && (
        <button
          ...
          disabled={busy}
          onClick={() => onTakeCharge(demande)}
        >
          <FiUserCheck aria-hidden="true" />
          {!compact && <span>Prendre</span>}
        </button>
      )}
      {!isClosed && (
        <button
...
export default function AdminDemandeTable({
  ...
  async function patchDemande(demande, changes) {
  ...
  async function handleTakeCharge(demande) {
    const changes = { assignedTo: currentUserId };
  ...
  async function handleClose(demande) {
    if (
      !window.confirm(
