...
function formatActivity(entry) {
  ...
  switch (entry.action) {
    case "creation": return "Demande créée";
    case "status_change": return `Statut : ${details.from} → ${details.to}`;
    case "message": return "Message ajouté";
    case "field_update": return "Champs modifiés";
    case "note_added": return "Note interne ajoutée";
    case "note_updated": return "Note interne modifiée";
    case "note_deleted": return "Note interne supprimée";
...
export default function AdminDemandeDetailPage({ params }) {
  ...
  async function load() {
    ...
    if (!demRes.ok) { setDemande(null); return; }
  ...
  async function handleMessage(e) {
    ...
    if (res.ok) { setContenu(""); await load(); }
  }
  ...
  async function handleNote(e) {
    ...
    const res = await fetch(`/api/demandes/${id}/notes`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contenu: noteContenu }),
    });
    if (res.ok) { setNoteContenu(""); await load(); }
  }
    ...
    >
      <div className="detail-grid">
        <section className="page-card">
          ...
          <form className="form-card form-card--inline" onSubmit={handleSave}>
            ...
            <div className="form-row">
              ...
              <div className="form-field">
                ...
                <select value={form.priorite} onChange={(e) => setForm({ ...form, priorite: e.target.value })}>
                  {DEMANDE_PRIORITES.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
                </select>
              </div>
            </div>
            <div className="form-row">
              <div className="form-field">
                ...
                <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  {Object.entries(DEMANDE_STATUTS).map(([v, i]) => <option key={v} value={v}>{i.label}</option>)}
                </select>
              </div>
              <div className="form-field">
                ...
                <select value={form.assignedTo} onChange={(e) => setForm({ ...form, assignedTo: e.target.value })}>
                  ...
                  {admins.map((a) => (
                    <option key={a.id} value={a.id}>{[a.prenom, a.nom].filter(Boolean).join(" ") || a.name}</option>
                  ))}
                </select>
              </div>
            </div>
            <div className="form-field">
              ...
              <select value={form.machineId} onChange={(e) => setForm({ ...form, machineId: e.target.value })}>
                ...
                {clientMachines.map((m) => <option key={m.id} value={m.id}>{m.nom}</option>)}
              </select>
            </div>
          ...
          {demande.last_activity_at && <p className="page-muted">Dernière activité : {formatDateTime(demande.last_activity_at)}</p>}
        </section>
        ...
        <section className="page-card">
          ...
          {demande.status !== "fermee" && (
            <form className="message-form" onSubmit={handleMessage}>
              <textarea value={contenu} onChange={(e) => setContenu(e.target.value)} rows={3} placeholder="Réponse au client…" required />
        ...
        <section className="page-card">
          ...
          <ul className="message-list">
            {notes.map((note) => (
              <li key={note.id} className="message-item message-item--note">
                <div className="message-header">
                  <strong>{note.auteur_prenom || note.auteur_nom || note.auteur_name}</strong>
                  <span className="page-muted">{formatDateTime(note.created_at)}</span>
                </div>
                <p>{note.contenu}</p>
              </li>
            ))}
          </ul>
          <form className="message-form" onSubmit={handleNote}>
            <textarea value={noteContenu} onChange={(e) => setNoteContenu(e.target.value)} rows={2} placeholder="Note interne (invisible client)…" />
            <button type="submit" className="btn btn-secondary">Ajouter une note</button>
          </form>
        </section>
