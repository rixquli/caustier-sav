...
export default function AdminFaqDetailPage({ params }: PageProps) {
  const router = useRouter();
  const [id, setId] = useState<string | null>(null);
  const [question, setQuestion] = useState("");
  const [reponse, setReponse] = useState("");
  const [categorie, setCategorie] = useState("");
  const [history, setHistory] = useState<FaqHistoryRow[]>([]);
  ...
  async function load() {
    ...
    if (!res.ok) return;
    const data = (await res.json()) as FaqDetailResponse;
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const data = (await res.json()) as UpdateFaqResponse | ApiErrorResponse;
      if (!res.ok) {
