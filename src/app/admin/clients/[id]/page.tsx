...
export default function AdminClientDetailPage({ params }: PageProps) {
  ...
  const [client, setClient] = useState<UserDisplay | null>(null);
  const [machines, setMachines] = useState<ClientMachineSummary[]>([]);
  const [demandes, setDemandes] = useState<DemandeDisplay[]>([]);
  const [machineForm, setMachineForm] = useState<MachineFormState>(EMPTY_MACHINE);
  ...
  async function load() {
    ...
    const data = (await clientRes.json()) as ClientDetailResponse;
  ...
  function openEditMachine(m: ClientMachineSummary) {
    setEditingMachineId(m.id);
    setMachineForm({
      ...EMPTY_MACHINE,
      nom: m.nom,
      marque: m.marque ?? "",
      produits_calibres: m.produits_calibres ?? "",
      version_logiciel: m.version_logiciel ?? "",
      date_mise_en_service: m.date_mise_en_service ?? "",
      pilote_ligne: m.pilote_ligne ?? "",
      technicien_charge: m.technicien_charge ?? "",
      nombre_lignes: m.nombre_lignes != null ? String(m.nombre_lignes) : "",
      serveurs_vision: m.serveurs_vision ?? "",
      notes_internes: m.notes_internes ?? "",
    });
    setShowMachineForm(true);
  }

  async function handleMachineSubmit(e: React.FormEvent) {
    e.preventDefault();
