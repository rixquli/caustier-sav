...
import type { ListClientsResponse, UserDisplay } from "@/types/user";

export default function AdminClientsPage() {
  const router = useRouter();
  const [clients, setClients] = useState<UserDisplay[]>([]);
  const [search, setSearch] = useState("");
  const [includeArchived, setIncludeArchived] = useState(false);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<UserDisplay | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/clients?${params}`)
      ...
      .then((data: ListClientsResponse) => setClients(data.clients ?? []))
