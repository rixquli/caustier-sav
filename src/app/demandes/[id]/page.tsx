...
export default function DemandeDetailPage({ params }: PageProps) {
  ...
  const [messages, setMessages] = useState<DemandeMessageRow[]>([]);
  ...
  async function load() {
    ...
    const data = (await res.json()) as DemandeDetailResponse;
  ...
  if (loading) {
    return (
      <PageLayout title="Demande" description="">
  ...
  if (!demande) {
    return (
      <PageLayout title="Introuvable" description="">
