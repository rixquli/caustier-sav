export type DemandeTypeOption = {
  value: string;
  label: string;
  badge: string;
};

export type DemandePrioriteOption = {
  value: string;
  label: string;
  badge: string;
};
...
export const DEMANDE_TYPES: DemandeTypeOption[] = [
...
export const DEMANDE_PRIORITES: DemandePrioriteOption[] = [
...
export const DEMANDE_STATUTS: Record<string, StatutInfo> = {
...
export function getTypeBadge(value: string): string {
  return (
    DEMANDE_TYPES.find((t) => t.value === value)?.badge ?? "badge-type--autre"
  );
}

export function getPrioriteLabel(value: string): string {
  return DEMANDE_PRIORITES.find((p) => p.value === value)?.label ?? value;
}

export function getPrioriteBadge(value: string): string {
  return (
    DEMANDE_PRIORITES.find((p) => p.value === value)?.badge ??
    "badge-prio--normale"
  );
}

export function getStatutInfo(status: string): StatutInfo {
