...
const knownIssuesPath = path.join(process.cwd(), "src/lib/known-issues.json");
...
function loadKnownIssues(): KnownIssue[] {
  try {
    return JSON.parse(fs.readFileSync(knownIssuesPath, "utf8")) as KnownIssue[];
  } catch {
    return [];
  }
}

function saveKnownIssues(issues: KnownIssue[]): void {
  fs.writeFileSync(
    knownIssuesPath,
...
function buildIssueMatchPrompt(
  problemText: string,
  entries: FaqMatchEntry[],
): string {
  const lines = [
...
export function buildModelPrompt(problemText: string): string {
  return buildIssueMatchPrompt(problemText, getKnownIssues());
}

export function buildFaqMatchPrompt(
  problemText: string,
  faqEntries: FaqMatchEntry[],
): string {
  return buildIssueMatchPrompt(problemText, faqEntries);
}

export function parseModelResponse(responseText: string): number {
...
export function parseFaqSuggestionResponse(
  responseText: string,
): FaqSuggestion | Record<string, never> | null {
...
export function buildFaqSuggestionMessages(conversationHistory: ConversationMessage[]) {
  const system = [
